#ifndef _AESALGO_HPP_
#define _AESALGO_HPP_
...
private:
	...
	static constexpr uint16_t blockBytesLen = 4 * Nb * sizeof(unsigned char);
	...
	void ShiftRow(std::array<std::array<uint8_t, Nb>, 4> &state,
				  uint16_t i,
				  uint16_t n); 

	void ShiftRows(std::array<std::array<uint8_t, Nb>, 4> &state);

	// multiply on x
	unsigned char xtime(unsigned char b); 

	void MixColumns(std::array<std::array<uint8_t, Nb>, 4> &state);

	void AddRoundKey(std::array<std::array<uint8_t, Nb>, 4> &state,
					 std::vector<uint8_t> &key);
	
	void InvSubBytes(std::array<std::array<uint8_t, Nb>, 4> &state);

	void InvMixColumns(std::array<std::array<uint8_t, Nb>, 4> &state);

	void InvShiftRows(std::array<std::array<uint8_t, Nb>, 4> &state);

	void SubWord(std::vector<uint8_t> &a);

	void RotWord(std::vector<uint8_t> &a);

	void XorWords(std::vector<uint8_t> &a,
	...
	uint8_t multiply_gf256(uint8_t a, uint8_t b);

	void KeyExpansion(std::vector<uint8_t> &key,
					  std::vector<uint8_t> &w);

	void EncryptBlock(std::vector<uint8_t> &in,
					  std::vector<uint8_t> &out,
					  std::vector<uint8_t> &roundKeys);

	void DecryptBlock(std::vector<uint8_t> &in,
					  std::vector<uint8_t> &out,
					  std::vector<uint8_t> &roundKeys);

	void XorBlocks(std::vector<uint8_t> &a, 
				   std::vector<uint8_t> &b,
				   std::vector<uint8_t> &c,
				   uint16_t len);

	std::vector<uint8_t> ArrayToVector(std::vector<uint8_t> &a, uint16_t len);

	std::vector<uint8_t> VectorToArray(std::vector<uint8_t> &a);
};

inline constexpr std::array<std::array<uint8_t, 16>, 16> sbox = {
	{
...
inline constexpr std::array<std::array<uint8_t, 16>, 16> inv_sbox = {
	{
...
inline constexpr std::array<std::array<uint8_t, 256>, 15> GF_MUL_TABLE = {
	{
		{{}},
		{{}},
...
inline constexpr std::array<std::array<uint8_t, 4>, 4> CMDS = {
	{
		{{2, 3, 1, 1}},
		{{1, 2, 3, 1}},
		{{1, 1, 2, 3}},
...
inline constexpr std::array<std::array<uint8_t, 4>, 4> INV_CMDS = {
	{
		{{14, 11, 13, 9}}, 
		{{9, 14, 11, 13}}, 
		{{13, 9, 14, 11}}, 
